...
def home():
    ...
    return "Car Service Agent Running"

@app.route('/voice', methods=['POST'])
def voice():
    """Initial voice endpoint - greet user"""
    r = VoiceResponse()
    text = "Namaste! Main Sarthi Toyota ki AI agent bol rahi hoon. Aapki car service ke liye call kar rahi hoon."
    
    log_interaction("N/A (call start)", text)
    
    # Use TTS for greeting
    generate_tts_and_play(r, text, "Hello! I am calling regarding your car service.")
    
    # Redirect to car number confirmation
...
def car_number():
    ...
    if car != 'unknown':
        text = f"aapkai care no {car} ka service pending hai jo 25/09/2025 ko khatm ho jaegi . Kya aap service book krna chahengay? ha ya na mai jawab"
    else:
        text = "Hum aapki car ka number confirm nahi kar pa rahe. Kya aap apna car number bata sakte hain?"
    
    log_interaction("Car number confirmation prompt", text)
    ...
    generate_tts_for_gather(gather, text, "Please confirm your car number")
...
def service():
    """Handle car number confirmation and show service due date"""
    ...
    log_interaction(speech, f"Processing confirmation for car: {car}")
    ...
    if is_yes(speech):
        ...
        if svc and 'next_service_date' in svc:
            ...
            text = f"Aapki car service ki due date {date_str} hai. Kya aap ye date reschedule karna chahte hai?"
        else:
            text = "Aapki car service next month due hai. Kya aap reschedule karna chahte hai?"
...
def reschedule():
    ...
    if is_yes(speech):
        text = "Bahut achha! Aap konse din  ko service schedule karna chahte hai? Kripya date bataiye."
...
def reschedule_date():
    ...
    text = f"Perfect! Aapki car service {formatted} ko schedule kar di gayi hai ."
...
def handle_services():
    ...
    if any(word in speech for word in ['nahi', 'nahin', 'no', 'nothing', 'kuch nahi']):
        ...
        # Final message
        final_text = "Aapka service appointment confirm ho gaya hai. SMS confirmation aapko mil jayega. Sarthi Toyota ko choose karne ke liye dhanyawad!"
    ...
    if chosen:
        ...
        text = f"Bahut badhiya! Aapne ye services select ki hai: {services_text}. Ye sab services add kar di gayi hai aur total cost ka estimate SMS mein aayega."
    ...
    final_text = "Aapka service appointment complete confirm ho gaya hai. Appointment details aur total estimate SMS mein mil jayega. Sarthi Toyota choose karne ke liye bahut bahut dhanyawad! Aapka din shubh ho!"
    generate_tts_and_play(r, final_text, "Your appointment is confirmed. Details will be sent via SMS. Thank you and have a great day!")
...
def end_call():
    ...
    text = ("Dhanyavaad! Aapne apna samay diya, iske liye shukriya. "
            "Aapki car service ki details hamne note kar li hai. "
            "Sarthi Toyota ki taraf se aapko ache din ki shubhkamnayein.")
    
    log_interaction("Call end", text)
    
    # Use TTS for ending message
    generate_tts_and_play(r, text, 
        "Thank you for your time. We have noted your car service details. "
        "On behalf of Sarthi Toyota, we wish you a wonderful day. Goodbye!")
...
if __name__ == "__main__":
    ...
    logger.info("Starting Car Service Agent server on port 5000")
