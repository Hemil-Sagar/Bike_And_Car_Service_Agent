...
SERVICES = [
    ...
    "Battery health check", "Car wash", "Coolant top-up",
...
async def home():
    ...
    return {"message": "Car Service Agent Running"}
...
async def voice():
    ...
    await generate_tts_and_play(r, text, "Hello! I am calling regarding your car service.")
...
async def car_number(To: str = Form(...)):
    ...
    r = VoiceResponse()
    text = f"Kya aapkai car ka number {car} hai? Kripya haan ya na mein jawab dein." if car != 'unknown' else "Hum aapki car ka number confirm nahi kar pa rahe. Kya aap apna car number bata sakte hain?"
    ...
    await generate_tts_for_gather(gather, text, "Please confirm your car number")
    r.append(gather)
...
async def startup_event():
    ...
    logger.info("Starting Car Service Agent server")
